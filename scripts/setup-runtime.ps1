...
$runtimeRoot = Assert-WithinJunyxRoot (Join-Path $root '.runtime\llama.cpp')
$downloadDir = Assert-WithinJunyxRoot (Join-Path $root '.runtime\downloads')
$modelsDir = Assert-WithinJunyxRoot (Join-Path $root 'models')
...
function Get-VerifiedFile([string]$Url, [string]$Destination, [string]$ExpectedSha256) {
    ...
    $partial = Assert-WithinJunyxRoot "$Destination.partial"
...
$release = Invoke-RestMethod -Uri $releaseUrl -Headers @{ Accept = 'application/vnd.github+json'; 'User-Agent' = 'JUNYX-Setup' }
...
foreach ($backendName in $backends) {
    ...
    $backendDir = Assert-WithinJunyxRoot (Join-Path $runtimeRoot $backendName)
    ...
    foreach ($assetName in $backendConfig.assets) {
        ...
        $zipPath = Assert-WithinJunyxRoot (Join-Path $downloadDir $assetName)
...
if (-not $SkipModel) {
    ...
    $tree = Invoke-RestMethod -Uri $treeUrl -Headers @{ 'User-Agent' = 'JUNYX-Setup' }
    ...
    $modelPath = Assert-WithinJunyxRoot (Join-Path $modelsDir $file)
    ...
    $setupMarker = Assert-WithinJunyxRoot (Join-Path $root '.runtime\setup-complete.json')
    @{
...
Write-Host 'JUNYX runtime setup completed.'
