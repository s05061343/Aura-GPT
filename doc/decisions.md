...
## ADR-002：MVP 使用 AI SDK 執行 Agent loop

- 狀態：Proposed
- 背景：原設計同時指定 AI SDK 與 LangChain，但兩者責任重疊，會增加串流與 tool event 轉換成本。
- 決策：MVP 暫由 AI SDK 負責串流與有限步數 tool loop，不預設引入 LangChain。
- 替代方案：LangChain/LangGraph；自製完整 loop。
- 重新評估條件：出現持久化工作流、複雜分支、人機協作 checkpoint 或多 Agent 需求。

## ADR-003：以 adapter 宣稱有限模型相容性

- 狀態：Proposed
- 背景：GGUF 只描述模型檔格式，無法保證 chat template、tools schema 或 JSON 行為一致。
- 決策：LLM adapter 只依賴經測試的 OpenAI-compatible API 子集，並以 model profile 和 smoke suite 管理支援範圍。
